 ...
 *
 * BYOK: _apiKey is REQUIRED — pass your own PredictHQ access token. Pipeworx
 * does not front a key, because PredictHQ sells a 14-day trial and then routes
 * you to a salesperson; there is no self-serve tier to share.
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'PredictHQ');
}
...
const API_KEY_PROP = {
  type: 'string',
  description:
    'REQUIRED — your own PredictHQ access token. Pipeworx does not front a key for PredictHQ (14-day trial, then sales contact). Free keyless alternatives for event listings: ticketmaster, seatgeek.',
} as const;
...
async function phqGet(path: string, apiKey: string): Promise<unknown> {
  ...
  if (res.status === 401) {
    return { __error: 'PredictHQ rejected the access token. Pipeworx does not front a key for PredictHQ — their free tier is a 14-day trial, after which it is sales-gated — so pass your own token as _apiKey. For event listings without a key, use ticketmaster instead.' };
  }
  if (res.status === 429) {
